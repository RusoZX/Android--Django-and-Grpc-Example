...
public class ContactosDjangoViewModel extends ViewModel {
    public final MutableLiveData<Integer> cantidad = new MutableLiveData<>();
    private Context contexto;
    public void setContexto(Context contexto){
        this.contexto=contexto;
    }
    public final LiveData<ArrayList<Contacto>> respuesta = Transformations.switchMap(cantidad,
            new Function<Integer, LiveData<ArrayList<Contacto>>>() {
                @Override
                public LiveData<ArrayList<Contacto>> apply(Integer input) {
                    try {
                        return new Manager(contexto).getContactosDjango(input);
                    } catch (Exception e) {
